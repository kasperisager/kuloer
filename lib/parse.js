...
export function parseRgb(str) {
  ...
  return [
    ...
    m[4] ? Math.max(0, Math.min(1, Math.round(parseFloat(m[4]) * 1e2) / 1e2)) : 1
  ];
}
...
export function parseHsl(str) {
  ...
  return [
    [
      ...
      Math.max(0, Math.min(1, Math.round(parseFloat(m[2])) / 1e2)),
      Math.max(0, Math.min(1, Math.round(parseFloat(m[3])) / 1e2))
    ],
    m[4] ? Math.max(0, Math.min(1, Math.round(parseFloat(m[4]) * 1e2) / 1e2)) : 1
  ];
}
