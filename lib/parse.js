...
export function parseRgb(str) {
  ...
  return [
    ...
    m[4] ? Math.max(0, Math.min(1, Math.round(Number(m[4]) * 1e2) / 1e2)) : 1
  ];
}
...
export function parseHsl(str) {
  ...
  return [
    [
      (r + 360) % 360,
      Math.max(0, Math.min(1, Math.round(g) / 1e2)),
      Math.max(0, Math.min(1, Math.round(b) / 1e2))
    ],
    m[4] ? Math.max(0, Math.min(1, Math.round(Number(m[4]) * 1e2) / 1e2)) : 1
  ];
}
