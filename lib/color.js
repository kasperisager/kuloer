 ...
 */
export function composite([backdrop, ...colors]) {
  if (!colors.length) {
    return backdrop;
  }

  const {round} = Math;
  let {rgb: [cr, cg, cb], alpha: ca} = backdrop;

  for (let i = 0, n = colors.length; i < n; i++) {
    const {rgb: [er, eg, eb], alpha: ea} = colors[i];
    const da = ca * (1 - ea);
  ...
  const rgb = [round(cr), round(cg), round(cb)];
  const hex = rgbToHex(rgb);
  const hsl = rgbToHsl(rgb);

  return {hex, rgb, hsl, alpha: round(ca * 1e2) / 1e2};
}
